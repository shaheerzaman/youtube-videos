...
async def fetch(session: aiohttp.ClientSession, url: str) -> dict[Any, Any]:
...
async def email_comments(number_of_comments: int) -> None:
...
async def post_number_of_comments(session: aiohttp.ClientSession, post_id: int) -> int:
    ...
    if number_of_comments > MIN_COMMENTS:
        await email_comments(number_of_comments)
